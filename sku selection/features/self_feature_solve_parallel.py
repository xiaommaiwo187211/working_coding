...
# target_type = "manjian" # (coupon, manjianzhe, manjian, meimanjian,2_stage_manjianzhe,2_stage_manjian)
...
# 历史nosplit表信息
yesterday = spark.sql('''select max(dt) from app.app_pa_performance_nosplit_self ''').collect()[0][0]
...
select 
    ...
    cann_sales,
    date as dt
from app.app_pa_performance_nosplit_self
where dt='%s'
and date >= '%s'
''' % (yesterday, origin_dt))
