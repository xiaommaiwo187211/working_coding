...
# nosplit取最新分区
latest_nosplit_dt =spark.sql('''select max(bu_id)  from dev.black_list_model_feature_books''').collect()[0][0]

# transactions_D_self
latest_trans_d_dt =spark.sql('''select max(bu_id) from dev.black_list_model_feature_books''').collect()[0][0]
...
# stage = '1_stage'  # [1_stage, 2_stage]
# # bu_id = '2019-04-26' # (bu_id= last partition of nosplit)
# target_type = 'meimanjian'  # (coupon, manjianzhe, manjian, meimanjian,2_stage_manjianzhe,2_stage_manjian)

# 目标：2个代码走天下，分为book和self；
if stage == "1_stage":
	# 读促销信息表-----by changxin
	df = spark.sql('''select * from app.books_promo_train_data where bu_id = "%s" and type ="%s"'''%(bu_id,target_type)).select('promotion_id')

if stage == "2_stage":
	# 读促销信息表-----by changxin
	df = spark.sql('''select * from app.books_promo_train_data_2_stages where bu_id = "%s" and type ="%s"'''%(bu_id,target_type)).select('promotion_id')
...
if stage == '1_stage':
	df = spark.sql(
		'''select * from dev.black_list_model_feature_books where bu_id = "%s" and type ="%s"''' % (bu_id, target_type))
	...
	if target_type == 'manjianzhe':
		df_current_info_finaly = df.fillna(0).drop('pricetime')
	df = df.withColumn('rank', F.col('uv').cast('int') % 15)
...
if stage == '2_stage':
	if target_type == '2_stage_manjian':
		df_promo = spark.sql('''select promo_id as promotion_id, threshold_money,
		        discount_money, threshold_num, discount_num, discount_rate, add_money from app.self_total_promo_rule_detail
		        where dt>'%s'
		        and dt<='%s'
		...
		rank_1 = rank.filter(F.col('threshold_money_rank') == 1).withColumnRenamed('threshold_money',
		                                                                           'threshold_num_first').withColumnRenamed(
			'discount_money', 'discount_rate_first')
		rank_2 = rank.filter(F.col('threshold_money_rank') == 2).withColumnRenamed('threshold_money',
		                                                                           'threshold_num_second').withColumnRenamed(
			'discount_money', 'discount_rate_second').drop('promo_stage')
		df_current_info_finaly_1 = rank_1.join(rank_2, 'promotion_id', 'inner')
		# 获取合适的promotion_id中的一阶和二阶促销信息；
		df_current_info_finaly_2 = df_current_info_finaly_1.select(
			['promotion_id', 'threshold_num_first', 'discount_rate_first', 'threshold_num_second',
			 'discount_rate_second', 'promo_stage'])
		df_history = spark.sql(
			'''select * from dev.black_list_model_feature_books where bu_id = "%s" and type ="%s"''' % (
			bu_id, target_type))

		df_current_info_finaly = df_current_info_finaly_2.join(df_history, 'promotion_id', 'inner')
		df = df_current_info_finaly.fillna(0).drop('promo_stage', 'promotion_id', 'consume_lim', 'cps_face_value',
		                                           'discount_rate_cal', 'pricetime')
	...
	if target_type == '2_stage_manjianzhe':
		df_promo = spark.sql('''select promo_id as promotion_id, threshold_money, discount_money, threshold_num, discount_num, discount_rate, add_money from app._total_promo_rule_detail
		where dt>'%s'
		and dt<='%s'
		...
		rank_1 = rank.filter(F.col('threshold_num_rank') == 1).withColumnRenamed('threshold_num',
		                                                                         'threshold_num_first').withColumnRenamed(
			'discount_rate', 'discount_rate_first')
		rank_2 = rank.filter(F.col('threshold_num_rank') == 2).withColumnRenamed('threshold_num',
		                                                                         'threshold_num_second').withColumnRenamed(
			'discount_rate', 'discount_rate_second').drop('promo_stage')
		df_current_info_finaly_1 = rank_1.join(rank_2, 'promotion_id', 'inner')
		# 获取合适的promotion_id中的一阶和二阶促销信息；
		df_current_info_finaly_2 = df_current_info_finaly_1.select(
			['promotion_id', 'threshold_num_first', 'discount_rate_first', 'threshold_num_second',
		...
		df = df_current_info_finaly.fillna(0).drop('promo_stage', 'promotion_id', 'consume_lim', 'cps_face_value',
		                                           'discount_rate_cal', 'pricetime')
		df = df.withColumn('first_stage_discount_rate', F.col('discount_rate_first')).withColumn(
			'second_stage_discount_rate', F.col('discount_rate_second'))
		df = df.withColumn('rank', F.col('uv').cast('int') % 15)
	df_0 = df.filter(F.col('rank') == 0).toPandas()
	df_1 = df.filter(F.col('rank') == 1).toPandas()
	df_2 = df.filter(F.col('rank') == 2).toPandas()
...
count_threshold = 300000
promo_type = target_type
if promo_type == '2_stage_manjianzhe':
	df['discount_flag'] = 'other'
	df.loc[(df['second_stage_discount_rate'] >= 0.85) & (
				df['second_stage_discount_rate'] < 0.95), 'discount_flag'] = '85_95'
	df.loc[(df['second_stage_discount_rate'] >= 0.75) & (
				df['second_stage_discount_rate'] < 0.85), 'discount_flag'] = '75_85'
	df.loc[(df['second_stage_discount_rate'] >= 0.65) & (
				df['second_stage_discount_rate'] < 0.75), 'discount_flag'] = '65_75'
	df.loc[(df['second_stage_discount_rate'] >= 0.55) & (
				df['second_stage_discount_rate'] < 0.65), 'discount_flag'] = '55_65'
	df.loc[(df['second_stage_discount_rate'] >= 0.45) & (
				df['second_stage_discount_rate'] < 0.55), 'discount_flag'] = '45_55'
	df.loc[df['second_stage_discount_rate'] < 0.45, 'discount_flag'] = '45_less'
elif promo_type == '2_stage_manjian':
	df['discount_flag'] = 'other'
	df.loc[(df['second_stage_discount_rate'] >= 0.05) & (
				df['second_stage_discount_rate'] < 0.15), 'discount_flag'] = '05_15'
	df.loc[(df['second_stage_discount_rate'] >= 0.15) & (
				df['second_stage_discount_rate'] < 0.25), 'discount_flag'] = '15_25'
	df.loc[(df['second_stage_discount_rate'] >= 0.25) & (
				df['second_stage_discount_rate'] < 0.35), 'discount_flag'] = '25_35'
	df.loc[(df['second_stage_discount_rate'] >= 0.35) & (
				df['second_stage_discount_rate'] < 0.45), 'discount_flag'] = '35_45'
	df.loc[(df['second_stage_discount_rate'] >= 0.45) & (
				df['second_stage_discount_rate'] < 0.55), 'discount_flag'] = '45_55'
	df.loc[df['second_stage_discount_rate'] >= 0.55, 'discount_flag'] = '55_more'
elif promo_type == 'manjianzhe':
	df['discount_flag'] = 'other'
	df.loc[(df['discount_rate_cal'] >= 0.85) & (df['discount_rate_cal'] < 0.95), 'discount_flag'] = '85_95'
	df.loc[(df['discount_rate_cal'] >= 0.75) & (df['discount_rate_cal'] < 0.85), 'discount_flag'] = '75_85'
	df.loc[(df['discount_rate_cal'] >= 0.65) & (df['discount_rate_cal'] < 0.75), 'discount_flag'] = '65_75'
	df.loc[(df['discount_rate_cal'] >= 0.55) & (df['discount_rate_cal'] < 0.65), 'discount_flag'] = '55_65'
	df.loc[(df['discount_rate_cal'] >= 0.45) & (df['discount_rate_cal'] < 0.55), 'discount_flag'] = '45_55'
	df.loc[df['discount_rate_cal'] < 0.45, 'discount_flag'] = '45_less'
else:
	df['discount_flag'] = 'other'
	df.loc[(df['discount_rate_cal'] >= 0.05) & (df['discount_rate_cal'] < 0.15), 'discount_flag'] = '05_15'
	df.loc[(df['discount_rate_cal'] >= 0.15) & (df['discount_rate_cal'] < 0.25), 'discount_flag'] = '15_25'
	df.loc[(df['discount_rate_cal'] >= 0.25) & (df['discount_rate_cal'] < 0.35), 'discount_flag'] = '25_35'
	df.loc[(df['discount_rate_cal'] >= 0.35) & (df['discount_rate_cal'] < 0.45), 'discount_flag'] = '35_45'
	df.loc[(df['discount_rate_cal'] >= 0.45) & (df['discount_rate_cal'] < 0.55), 'discount_flag'] = '45_55'
	df.loc[df['discount_rate_cal'] >= 0.55, 'discount_flag'] = '55_more'

import upload_to_oss

df_count = df.groupby('discount_flag').agg({'item_sku_id': np.size}).reset_index()
model_flag_list = list(df_count.loc[df_count['item_sku_id'] >= count_threshold, 'discount_flag'])
for flag in model_flag_list:
	new_df = df.loc[df['discount_flag'] == flag]
	training, test = train_test_split(new_df, test_size=0.2, random_state=1)

	name['rf_model_%s' % flag] = RandomForestClassifier(n_estimators=200, criterion='gini', max_depth=10,
	                                                    oob_score=True, max_features='sqrt', random_state=1)
	name['model_fit_%s' % flag] = eval('rf_model_%s' % flag).fit(training[feature_col_feature], training['label'])
	from sklearn.externals import joblib

	joblib.dump(eval('model_fit_%s' % flag), "book_%s_rf_model_%s.m" % (promo_type, flag))
	upload_to_oss.model_upload("book_%s_rf_model_%s.m" % (promo_type, flag),latest_nosplit_dt+"deal_price")
	# 随机森林结果
	threshold = 0.7

	predicted_proba = eval('model_fit_%s' % flag).predict_proba(test[feature_col_feature])
	predicted = (predicted_proba[:, 1] >= threshold).astype('int')

	fitting_test_df = pd.DataFrame(predicted).rename(columns={0: 'fit'})
	tt_y = pd.DataFrame(list(test['item_sku_id'])).rename(columns={0: 'fact'})
	tt = pd.concat([tt_y, fitting_test_df], axis=1)
	black_rate = sum(df['label']) / len(df)
	record.loc[0] = ['RF_7', precision_score(test['label'], predicted), recall_score(test['label'], predicted),
	                 sum(predicted), black_rate]
	...
	tt = pd.concat([tt_y, fitting_test_df], axis=1)
	black_rate = sum(df['label']) / len(df)
	record.loc[1] = ['RF_6', precision_score(test['label'], predicted), recall_score(test['label'], predicted),
	                 sum(predicted), black_rate]

	record_spark = spark.createDataFrame(record)
	record_spark = record_spark.withColumn('bu_id', F.lit(bu_id)).withColumn('type', F.lit(target_type)) \
		.withColumn('discount_flag', F.lit(flag))
	record_col = ['model', 'precision', 'recall', 'black_count', 'black_rate', 'discount_flag', 'bu_id', 'type']
	table_name = 'dev.dev_black_selection_model_record_book_discount_size'
	record_spark.select(record_col).write.insertInto(table_name, overwrite=True)