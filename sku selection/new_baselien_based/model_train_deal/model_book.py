...
# target_type = 'coupon' # (coupon, manjianzhe, manjian, meimanjian,2_stage_manjianzhe,2_stage_manjian)
...
if stage == '1_stage':
    df = spark.sql('''select * from dev.black_list_model_feature_books where bu_id = "%s" and type ="%s"'''%(bu_id,target_type))
...
if stage == '2_stage':
    if target_type == '2_stage_manjian':
        ...
        df_history = spark.sql('''select * from dev.black_list_model_feature_books where bu_id = "%s" and type ="%s"'''%(bu_id,target_type))
...
table_name = 'dev.dev_black_selection_model_record_book_self'
record.select(record_col).write.insertInto(table_name, overwrite=True)

from sklearn.externals import joblib
joblib.dump(model_fit_noweight, "book_%s_rf_model.m"%(target_type))
import upload_to_oss
upload_to_oss.model_upload('book_%s_rf_model.m'%(target_type),latest_nosplit_dt+"deal_price")
# import os
# os.system('hadoop fs -put book_%s_rf_model.m /user/mart_rmb/user/xiaoxiao10/black_list_model/edition_%s'%(target_type,bu_id))
