...
latest_nosplit_dt = spark.sql('''select max(bu_id)  from dev.black_list_model_feature_self''').collect()[0][0]

# transactions_D_self
latest_trans_d_dt = spark.sql('''select max(bu_id) from dev.black_list_model_feature_self''').collect()[0][0]
...
if stage == "1_stage":
	# 读促销信息表-----by changxin
	df = spark.sql(
		'''select * from app.self_promo_train_data where bu_id = "%s" and type ="%s"''' % (bu_id, target_type)).select(
		'promotion_id')

if stage == "2_stage":
	# 读促销信息表-----by changxin
	df = spark.sql('''select * from app.self_promo_train_data_2_stages where bu_id = "%s" and type ="%s"''' % (
	bu_id, target_type)).select('promotion_id')
...
if stage == '1_stage':
	df = spark.sql(
		'''select * from dev.black_list_model_feature_self where bu_id = "%s" and type ="%s"''' % (bu_id, target_type))
...
if stage == '2_stage':
	if target_type == '2_stage_manjian':
		...
		df_history = spark.sql(
			'''select * from dev.black_list_model_feature_self where bu_id = "%s" and type ="%s"''' % (
			bu_id, target_type))
...
for flag in model_flag_list:
	...
	joblib.dump(eval('model_fit_%s' % flag), "self_%s_rf_model_%s.m" % (promo_type, flag))
	upload_to_oss.model_upload("self_%s_rf_model_%s.m" % (promo_type, flag),latest_nosplit_dt+"deal_price")
	# 随机森林结果
	threshold = 0.7
	...
	table_name = 'dev.dev_black_selection_model_record_self_discount_size'
