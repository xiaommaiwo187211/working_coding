...
# nosplit取最新分区
latest_nosplit_dt = spark.sql('''select max(bu_id)  from dev.black_list_parallel_model_feature_self''').collect()[0][0]

# transactions_D_self
latest_trans_d_dt = spark.sql('''select max(bu_id) from dev.black_list_parallel_model_feature_self''').collect()[0][0]
...
target_type_parallel = sys.argv[3]
# stage = '1_stage' #[1_stage, 2_stage]
# bu_id = '2019-03-22' # (bu_id= last partition of nosplit)
# target_type = 'manjianzhe' # (coupon, manjianzhe, manjian, meimanjian,2_stage_manjianzhe,2_stage_manjian)
# target_type_parallel = "manjianzhe_parallel"
# 目标：2个代码走天下，分为book和self；
if stage == "1_stage":
	# 读促销信息表-----by changxin
	df = spark.sql('''select * from dev.dev_parellel_pormo_basic where bu_id = "%s" and type ="%s"'''%(bu_id,target_type)).select('promotion_id','batch_id')
...
if stage == '1_stage':
	#     df = spark.sql('''select * from dev.black_list_parallel_model_feature_self where bu_id = "%s" and type ="%s"'''%(bu_id,target_type))
	if target_type == 'manjian':
		...
		df_xx = df.select('promotion_id').distinct()

		df_promo = df_promo.distinct()
		df_final = df_xx.join(df_promo, 'promotion_id', 'inner')
		...
		df_current_info_finaly_1 = df_final_2.join(df_promo, 'promotion_id', 'left')
		df_current_info_finaly_2 = df_current_info_finaly_1.withColumnRenamed('threshold_money',
		                                                                      'threshold_num_first').withColumnRenamed(
			'discount_money', 'discount_rate_first')

		sql_batch = spark.sql(
			'''
			select batch_id,cps_face_value,consume_lim,first_ratio,
		...
		df_current_info_finaly_2_1 = df_current_info_finaly_2.join(df, 'promotion_id', 'inner')

		df_current_info_finaly_3 = df_current_info_finaly_2_1.join(sql_batch, 'batch_id', 'inner')
		df_current_info_finaly_4 = df_current_info_finaly_3.withColumnRenamed('consume_lim',
		                                                                      'batch_comsume').withColumnRenamed(
			'cps_face_value', 'batch_face')

		df_current_info_finaly_5 = df_current_info_finaly_4.select(
			['promotion_id', 'batch_id', 'threshold_num_first', 'discount_rate_first', 'batch_comsume', 'batch_face',
			 'promo_stage'])
		df_history = spark.sql(
			'''select * from dev.black_list_parallel_model_feature_self where bu_id = "%s" and type ="%s"''' % (
			bu_id, target_type_parallel))
		df_current_info_finaly = df_current_info_finaly_5.join(df_history, ['promotion_id', 'batch_id'], 'inner')

		# drop some columns
		df = df_current_info_finaly.fillna(0).drop('promo_stage', 'promotion_id', 'batch_id', 'consume_lim',
		                                           'cps_face_value', 'discount_rate_cal', 'pricetime')
		df = df.withColumn('first_stage_discount_rate', F.col('discount_rate_first') / F.col('threshold_num_first')) \
			.withColumn('batch_rate', F.col('batch_face') / F.col('batch_comsume'))
		df = df.withColumn('rank', F.col('uv').cast('int') % 15)
		df_current_info_finaly = df.fillna(0)
	...
	if target_type == 'manjianzhe':
		...
		df_final_2 = df_final_1.filter(F.col('promo_stage') == 1)
		df_current_info_finaly_1 = df_final_2.join(df_promo, 'promotion_id', 'left')
		df_current_info_finaly_2 = df_current_info_finaly_1.withColumnRenamed('threshold_num',
		                                                                      'threshold_num_first').withColumnRenamed(
		...
		df = df_current_info_finaly.fillna(0).drop('promo_stage', 'promotion_id', 'batch_id', 'consume_lim',
		                                           'cps_face_value', 'discount_rate_cal', 'pricetime')
		df = df.withColumn('first_stage_discount_rate', F.col('discount_rate_first')) \
			.withColumn('batch_rate', F.col('batch_face') / F.col('batch_comsume'))
		df = df.withColumn('rank', F.col('uv').cast('int') % 15)
		df_current_info_finaly = df.fillna(0)
...
if stage == '2_stage':
	if target_type == '2_stage_manjian':
		...
		df_current_info_finaly_2 = rank_1.join(rank_2, 'promotion_id', 'inner')
		sql_batch = spark.sql(
			'''
			select batch_id,cps_face_value,consume_lim,first_ratio,
			datediff(valid_end_tm,valid_start_tm) as days
		...
		df_current_info_finaly_5 = df_current_info_finaly_4.select(
			['promotion_id', 'batch_id', 'threshold_num_first', 'discount_rate_first', 'threshold_num_second',
			 'discount_rate_second', 'batch_comsume', 'batch_face', 'promo_stage'])
		df_history = spark.sql(
			'''select * from dev.black_list_parallel_model_feature_self where bu_id = "%s" and type ="%s"''' % (
			bu_id, target_type_parallel))
		df_current_info_finaly = df_current_info_finaly_5.join(df_history, ['promotion_id', 'batch_id'], 'inner')
	...
	if target_type == '2_stage_manjianzhe':
		...
		rank = df_current_info.withColumn("threshold_money_rank", F.rank().over(w))
		# 此处的阶梯满减的first=一阶的门槛，second=二阶门槛
		rank_1 = rank.filter(F.col('threshold_money_rank') == 1).withColumnRenamed('threshold_num',
		                                                                           'threshold_num_first').withColumnRenamed(
			'discount_rate', 'discount_rate_first')
		rank_2 = rank.filter(F.col('threshold_money_rank') == 2).withColumnRenamed('threshold_num',
		                                                                           'threshold_num_second').withColumnRenamed(
		...
		df = df.withColumn('first_stage_discount_rate', F.col('discount_rate_first')) \
			.withColumn('batch_rate', F.col('batch_face') / F.col('batch_comsume')) \
			.withColumn('second_stage_discount_rate', F.col('discount_rate_second'))
		df = df.withColumn('rank', F.col('uv').cast('int') % 15)
		df_current_info_finaly = df.fillna(0)
	df_0 = df.filter(F.col('rank') == 0).toPandas()
	df_1 = df.filter(F.col('rank') == 1).toPandas()
...
table_name = 'dev.dev_black_selection_parallel_model_record_self'
record.select(record_col).write.insertInto(table_name, overwrite=True)


from sklearn.externals import joblib
joblib.dump(model_fit_noweight, "self_%s_rf_parallel_model.m"%(target_type))

import upload_to_oss
upload_to_oss.model_upload('self_%s_rf_parallel_model.m'%(target_type),latest_nosplit_dt+"deal_price")
