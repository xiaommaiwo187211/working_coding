...
if stage == "1_stage":
	# 读促销信息表-----by changxin
	chang_info = spark.sql('''select * from dev.dev_parellel_pormo_basic''')
	# 读本次促销sku表现数据
	label = spark.sql('''select * from dev.dev_parellel_pormo_detail''')
...
base_info = chang_info.join(label, ['bu_id', 'type', 'promotion_id', 'batch_id'], 'inner')
...
# 调整成为与非平行促销的格式
base_info = base_info.withColumnRenamed('threshold_money_coupon', 'consume_lim').withColumnRenamed('discount_money_coupon', 'cps_face_value')

# sku池子与促销信息(相关数据都不正确)
df_sql_skus = base_info.select('item_sku_id', 'promotion_id', 'batch_id', 'min_dt', 'max_dt', 'start_str_chanpin',
                                'end_str_chanpin', 'pricetime', 'consume_lim', 'cps_face_value').distinct()

df_sql_skus.write.insertInto('dev.dev_sku_tempo_xiaoxiao_%s'%(target_type), overwrite= True)
...
select
    ...
    item_sku_id, 
    final_baseline
from app.app_pa_baseline_sku_self
where dt = '%s'
and date >='%s'
''' % (yesterday, origin_dt))
...
# 历史基线部分
df_baseline = df_sql_skus.select('item_sku_id', 'promotion_id', 'batch_id','start_str_chanpin', 'end_str_chanpin').join(
	...
	.select(df_sql_skus.item_sku_id, df_sql_skus.promotion_id,df_sql_skus.batch_id, df_baseline_history.dt, df_baseline_history.baseqtty) \
	.withColumn('baseline_success', F.lit(1)) \
	.select('dt', 'item_sku_id', 'promotion_id', 'batch_id','baseline_success')
...
df_sql_sku_day_mess_xx = df_sql_skus.select('item_sku_id', 'promotion_id', 'batch_id','start_str_chanpin', 'end_str_chanpin').join(
...
df_sql_sku_day_mess_xx = df_sql_sku_day_mess_xx.groupby(
	['item_sku_id', 'promotion_id', 'batch_id','end_str_chanpin', 'his_promotion_id', 'his_batch_id', 'dt']).agg(
...
df_his_promo = df_sql_skus.select('item_sku_id', 'promotion_id', 'batch_id','end_str_chanpin').withColumn('start_str_30',
                                                                                               F.date_sub(F.col(
	                                                                                               'end_str_chanpin'),
                                                                                                          30)).withColumn(
...
df_his_promo_uv = df_sql_skus.select('item_sku_id', 'promotion_id','batch_id').join(df_his_promo_uv_0_30,
                                                                         ['item_sku_id', 'promotion_id','batch_id'], 'left').join(
	df_his_promo_uv_0_90, ['item_sku_id', 'promotion_id','batch_id'], 'left').join(df_his_promo_uv_0_365,
                                                                        ['item_sku_id', 'promotion_id','batch_id'], 'left').join(
	df_his_promo_uv_90_365, ['item_sku_id', 'promotion_id','batch_id'], 'left').fillna(0)
...
df_his_promo_red = df_sql_skus.select('item_sku_id', 'promotion_id','batch_id').join(df_his_promo_red_0_30,
                                                                          ['item_sku_id', 'promotion_id','batch_id'], 'left').join(
	df_his_promo_red_0_90, ['item_sku_id', 'promotion_id','batch_id'], 'left').join(df_his_promo_red_0_365,
                                                                         ['item_sku_id', 'promotion_id','batch_id'], 'left').join(
	df_his_promo_red_90_365, ['item_sku_id', 'promotion_id','batch_id'], 'left').fillna(0)
...
df_his_promo_base = df_sql_skus.select('item_sku_id', 'promotion_id','batch_id').join(df_his_promo_base_0_30,
                                                                           ['item_sku_id', 'promotion_id','batch_id'],
                                                                           'left').join(df_his_promo_base_0_90,
                                                                                        ['item_sku_id', 'promotion_id','batch_id'],
                                                                                        'left').join(
	df_his_promo_base_0_365, ['item_sku_id', 'promotion_id','batch_id'], 'left').join(df_his_promo_base_90_365,
                                                                           ['item_sku_id', 'promotion_id','batch_id'],
                                                                           'left').fillna(0)

df_his_promo = df_his_promo_uv.join(df_his_promo_red, ['item_sku_id', 'promotion_id','batch_id'], 'inner').join(df_his_promo_base,
                                                                                                     ['item_sku_id',
                                                                                                      'promotion_id','batch_id'],
                                                                                                     'inner')
...
df_sql_skus = spark.sql('''select * from dev.dev_sku_tempo_xiaoxiao_%s'''%(target_type))

df_cur_promo_1 = df_sql_skus.select('item_sku_id','promotion_id','consume_lim','cps_face_value','batch_id')\
                            .withColumn('discount_rate_cal',F.col('cps_face_value')/F.col('consume_lim'))

# df_cur_promo_1.cache()
# df_cur_promo_1.show()
df_sql_skus = spark.sql('''select * from dev.dev_sku_tempo_xiaoxiao_%s'''%(target_type))
df_cur_promo_2 = df_sql_skus.select('item_sku_id','promotion_id','pricetime','batch_id')\
...
# df_cur_promo_2.cache()
# df_cur_promo_2.show()

df_sql_skus = spark.sql('''select * from dev.dev_sku_tempo_xiaoxiao_%s'''%(target_type))
df_cur_promo_tmp = df_sql_skus.select('item_sku_id','promotion_id','pricetime','batch_id')\
                            .withColumn('pricetime_7',F.date_sub(F.col('pricetime'),days_para))
# df_cur_promo_tmp.cache()
# df_cur_promo_tmp.show()
...
df_cur_promo_3_uv = df_cur_promo_tmp.join(uv_info,(df_cur_promo_tmp.item_sku_id == uv_info.item_sku_id)&
                                    ...
                                    .drop(uv_info['dt'])\
                                    .groupby('item_sku_id','promotion_id','batch_id')\
                                    .agg(F.mean('uv').alias('uv'))\
                                    .fillna(0)
# df_cur_promo_3_uv.cache()
# df_cur_promo_3_uv.show()
...
# df_cur_promo_3_qtty.cache()
# df_cur_promo_3_qtty.show()
...
# df_cur_promo_3_qtty_2.cache()
# df_cur_promo_3_qtty_2.show()
df_sql_skus = spark.sql('''select * from dev.dev_sku_tempo_xiaoxiao_%s'''%(target_type))
df_cur_promo_4 = df_sql_skus.select('item_sku_id','promotion_id','min_dt','max_dt','batch_id')\
                            ...
                            .sort(['item_sku_id','promotion_id','batch_id','dt'])\
                            .groupby('item_sku_id','promotion_id','batch_id')\
...
# df_cur_promo_4.cache()
# df_cur_promo_4.show()
#.filter((F.col('red_price').isNotNull()) & (F.col('baseprice').isNotNull()))\
df_cur_promo = df_cur_promo_1.join(df_cur_promo_2,['promotion_id','item_sku_id','batch_id'],'inner').join(df_cur_promo_3_uv,['promotion_id','item_sku_id','batch_id'],'inner')\
                             .join(df_cur_promo_3_qtty_2,['promotion_id','item_sku_id','batch_id'],'inner')\
                             .join(df_cur_promo_4,['promotion_id','item_sku_id','batch_id'],'inner')

# 特征加工
feature_1 = df_sql_sku_day_mess_xx.join(df_baseline, ['item_sku_id', 'promotion_id','batch_id', 'dt'], 'inner').filter(
...
status_info = df_sql_skus.select('item_sku_id', 'promotion_id','batch_id', 'start_str_chanpin', 'end_str_chanpin').join(
...
for x, y in zip([0, 0, 0, 90], [30, 90, 365, 365]):
	name['status_%s_%s' % (x, y)] = status_info.filter(F.col('flag_%s_%s' % (x, y)) == 1).groupby('item_sku_id','promotion_id','batch_id').\
		agg(F.count('sku_status_cd').alias('status_%s_%s' % (x, y)))
	name['feature_%s_%s' % (x, y)] = eval('status_%s_%s' % (x, y)).join(
		feature_1.filter(F.col('flag_%s_%s' % (x, y)) == 1), ['item_sku_id', 'promotion_id','batch_id'], 'left').groupby(
		'item_sku_id', 'promotion_id','batch_id', 'discount_level_history').agg(
...
feature_2 = feature_1.select('item_sku_id', 'promotion_id','batch_id', 'discount_level_history').distinct() \
	.join(feature_0_30, ['item_sku_id', 'promotion_id','batch_id', 'discount_level_history'], 'left') \
	.join(feature_0_90, ['item_sku_id', 'promotion_id','batch_id', 'discount_level_history'], 'left') \
	.join(feature_0_365, ['item_sku_id', 'promotion_id','batch_id', 'discount_level_history'], 'left') \
	.join(feature_90_365, ['item_sku_id', 'promotion_id','batch_id', 'discount_level_history'], 'left') \
	.fillna(0)
...
feature_3 = feature_2.groupby('item_sku_id', 'promotion_id','batch_id') \
    .pivot('discount_level_history', ['10%_less', '10%_20%', '20%_30%', '30%_40%', '40%_50%', '50%_more']) \
    .agg(*cols).fillna(0)

df_sql_sku_day_mess_xx.unpersist()

model_data = feature_3.join(df_his_promo, ['item_sku_id', 'promotion_id','batch_id'], 'inner') \
	.join(df_cur_promo, ['item_sku_id', 'promotion_id','batch_id'], 'inner') \
	.join(base_info.select('item_sku_id', 'promotion_id','batch_id', 'label'), ['item_sku_id', 'promotion_id','batch_id'], 'inner')

model_data = model_data.withColumn('bu_id', F.lit(bu_id)).withColumn('type', F.lit(target_type+"_parallel"))
...
model_data.write.insertInto('dev.black_list_parallel_model_feature_self', overwrite=True)
