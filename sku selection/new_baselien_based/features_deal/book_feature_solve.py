...
if stage == "1_stage":
	# 读促销信息表-----by changxin
	chang_info = spark.sql('''select * from app.books_promo_train_data''')
	# 读本次促销sku表现数据
	label = spark.sql('''select * from dev.books_train_dataset''')


if stage == "2_stage":
	# 读促销信息表-----by changxin
	chang_info = spark.sql('''select * from app.books_promo_train_data_2_stages''')
	# 读本次促销sku表现数据
	label = spark.sql('''select * from dev.books_train_dataset_2_stages''')
...
# 日期处理
base_info = base_info.withColumn('start_str_chanpin', F.date_sub(F.col('min_dt'), 375)) \
	.withColumn('end_str_chanpin', F.date_sub(F.col('min_dt'), 10)) \
	.withColumn('pricetime', F.date_sub(F.col('min_dt'), 7))
...
where (sku_type = 2 or sku_type = 3)
and dt>='%s'
''' % origin_dt)
...
# 历史基线部分
df_baseline = df_sql_skus.select('item_sku_id', 'promotion_id', 'start_str_chanpin', 'end_str_chanpin') \
	.join(df_baseline_history, (df_sql_skus.item_sku_id == df_baseline_history.item_sku_id) &
...
performance_2 = performance.filter(
	(F.col('his_batch_id').isNull()) | (F.col('his_batch_id') == 0) | (F.col('his_batch_id') == '0')) \
	.join(df_promo_info_all, 'his_promotion_id', 'inner')

sql_day_col = performance_1.columns
performance = performance_1.select(sql_day_col) \
	.union(performance_2.select(sql_day_col))

df_sql_sku_day_mess_xx = df_sql_skus.select('item_sku_id', 'promotion_id', 'start_str_chanpin', 'end_str_chanpin') \
	.join(performance, (df_sql_skus.item_sku_id == performance.item_sku_id) &
...
# sku dt维度
df_sql_sku_day_mess_xx = df_sql_sku_day_mess_xx \
	.groupby(['item_sku_id', 'promotion_id', 'end_str_chanpin', 'his_promotion_id', 'his_batch_id', 'dt']) \
	.agg(
...
df_his_promo = df_sql_skus.select('item_sku_id', 'promotion_id', 'end_str_chanpin') \
	.withColumn('start_str_30', F.date_sub(F.col('end_str_chanpin'), 30)) \
	.withColumn('start_str_90', F.date_sub(F.col('end_str_chanpin'), 90)) \
	.withColumn('start_str_365', F.date_sub(F.col('end_str_chanpin'), 365))

# =====================================历史uv================================================
df_his_promo_uv_1 = df_his_promo.join(uv_info, 'item_sku_id', 'left')

df_his_promo_uv_0_30 = df_his_promo_uv_1.filter(
	(F.col('dt') >= F.col('start_str_30')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('uv').alias('his_uv_0_30'))

df_his_promo_uv_0_90 = df_his_promo_uv_1.filter(
	(F.col('dt') >= F.col('start_str_90')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('uv').alias('his_uv_0_90'))

df_his_promo_uv_0_365 = df_his_promo_uv_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('uv').alias('his_uv_0_365'))

df_his_promo_uv_90_365 = df_his_promo_uv_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('start_str_90'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('uv').alias('his_uv_90_365'))

df_his_promo_uv = df_sql_skus.select('item_sku_id', 'promotion_id') \
	.join(df_his_promo_uv_0_30, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_uv_0_90, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_uv_0_365, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_uv_90_365, ['item_sku_id', 'promotion_id'], 'left') \
	.fillna(0)
...
df_his_promo_red_0_30 = df_his_promo_red_1.filter(
	(F.col('dt') >= F.col('start_str_30')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('red_price').alias('his_red_price_0_30'))

df_his_promo_red_0_90 = df_his_promo_red_1.filter(
	(F.col('dt') >= F.col('start_str_90')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('red_price').alias('his_red_price_0_90'))

df_his_promo_red_0_365 = df_his_promo_red_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('red_price').alias('his_red_price_0_365'))

df_his_promo_red_90_365 = df_his_promo_red_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('start_str_90'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('red_price').alias('his_red_price_90_365'))

df_his_promo_red = df_sql_skus.select('item_sku_id', 'promotion_id') \
	.join(df_his_promo_red_0_30, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_red_0_90, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_red_0_365, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_red_90_365, ['item_sku_id', 'promotion_id'], 'left') \
	.fillna(0)
...
df_his_promo_base_0_30 = df_his_promo_base_1.filter(
	(F.col('dt') >= F.col('start_str_30')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('baseprice').alias('his_baseprice_0_30'))

df_his_promo_base_0_90 = df_his_promo_base_1.filter(
	(F.col('dt') >= F.col('start_str_90')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('baseprice').alias('his_baseprice_0_90'))

df_his_promo_base_0_365 = df_his_promo_base_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('end_str_chanpin'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('baseprice').alias('his_baseprice_0_365'))

df_his_promo_base_90_365 = df_his_promo_base_1.filter(
	(F.col('dt') >= F.col('start_str_365')) & (F.col('dt') <= F.col('start_str_90'))) \
	.groupby('item_sku_id', 'promotion_id') \
	.agg(F.mean('baseprice').alias('his_baseprice_90_365'))

df_his_promo_base = df_sql_skus.select('item_sku_id', 'promotion_id') \
	.join(df_his_promo_base_0_30, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_base_0_90, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_base_0_365, ['item_sku_id', 'promotion_id'], 'left') \
	.join(df_his_promo_base_90_365, ['item_sku_id', 'promotion_id'], 'left') \
	.fillna(0)

df_his_promo = df_his_promo_uv.join(df_his_promo_red, ['item_sku_id', 'promotion_id'], 'inner') \
	.join(df_his_promo_base, ['item_sku_id', 'promotion_id'], 'inner')
...
# 特征加工
feature_1 = df_sql_sku_day_mess_xx.join(df_baseline, ['item_sku_id', 'promotion_id', 'dt'], 'inner') \
	.filter(F.col('baseline_success') == 1) \
	.drop(df_baseline['baseline_success']) \
	.withColumn('discount_rate_history', F.col('total_giveaway') / (F.col('baseline_sales') + F.col('uplift_sales')))
...
status_info = df_sql_skus.select('item_sku_id', 'promotion_id', 'start_str_chanpin', 'end_str_chanpin') \
	.join(df_sql_sku_status.filter(F.col('sku_status_cd') == 3001),
...
for x, y in zip([0, 0, 0, 90], [30, 90, 365, 365]):
	name['status_%s_%s' % (x, y)] = status_info.filter(F.col('flag_%s_%s' % (x, y)) == 1) \
		.groupby('item_sku_id', 'promotion_id') \
		.agg(F.count('sku_status_cd').alias('status_%s_%s' % (x, y)))

	name['feature_%s_%s' % (x, y)] = eval('status_%s_%s' % (x, y)) \
		.join(feature_1.filter(F.col('flag_%s_%s' % (x, y)) == 1), ['item_sku_id', 'promotion_id'], 'left').groupby('item_sku_id', 'promotion_id', 'discount_level_history') \
		.agg((F.sum('sale_qtty') / F.max('status_%s_%s' % (x, y))).alias('sale_qtty_%s_%s' % (x, y)),\
	         ...
	         ((F.sum('uplift_sales') - F.sum('total_giveaway')) / F.sum('baseline_sales')).alias(\
		         'incre_%s_%s' % (x, y)))
...
table_name = 'dev.black_list_model_feature_books'  # 表名需要更改，先建表，再存数据
