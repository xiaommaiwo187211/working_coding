...
# transactions_D_self
latest_trans_d_dt = latest_nosplit_dt
...
target_type = '2_stage_manjianzhe'
...
distinct_sku = spark.sql('''
    ...
    app.app_pa_performance_nosplit_self_deal_price
    where dt= '%s'
...
df_promo_all = df_promo_all.withColumn('bu_id', F.lit(refresh_dt))
...
table_name = 'dev.self_train_dataset_2_stages'  # 表名需要更改，先建表，再存数据
partitioning_columns = ['bu_id', 'type']
df.write.insertInto(table_name, overwrite=True)

promo100_info_1 = promo100_info.withColumn('cps_face_value', F.col('threshold_num'))
promo100_info_1 = promo100_info_1.withColumn('consume_lim', F.col('discount_rate'))
...
table_name = 'app.self_promo_train_data_2_stages'  # 表名需要更改，先建表，再存数据
partitioning_columns = ['bu_id', 'type']
