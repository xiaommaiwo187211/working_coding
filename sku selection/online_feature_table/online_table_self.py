...
spark = SparkSession.builder.appName("online_table_self").enableHiveSupport().getOrCreate()
...
end_dt = spark.sql('''select max(date) from app.app_pa_performance_nosplit_self_deal_price where dt = '%s' '''%yesterday).collect()[0][0]
...
# sku池子
df_sql_skus = performance.select('item_sku_id').distinct()\
                         .withColumn('promotion_id',F.lit('-9999'))\
                         .withColumn('min_dt',F.lit(end_dt))
...
save_result(model_data,'dev.dev_empty_bottle_black_selection_self',partitioning_columns=['dt'], write_mode='insert')
# save_result(model_data,'dev.dev_empty_bottle_black_selection_self',partitioning_columns=['dt'], write_mode='save')
